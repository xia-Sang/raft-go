...
func (rf *Raft) becomeFollowerLock(term int) {
	...
	LOG(rf.me, rf.currentTerm, DLog, "%s->Follower, for T%s->T%s", rf.role, rf.currentTerm)
	rf.role = Follower
	if term > rf.currentTerm {
		rf.votedFor = -1
	}
}
func (rf *Raft) becomeCandidateLoced() {
	if rf.role == Leader {
...
func(rf*Raft)startElection(term int){
	...
	askVoteFromPeer:=func(peer int ,args*RequestVoteArgs){
		...
		if rf.contextLostLocked(Candidate,term){
			LOG(rf.me,rf.currentTerm,DVote,"Lost context, abort ReqestVoteReply for S%d",peer)
			return 
		}
		// count the votes
	...
	defer rf.mu.Unlock()
	if rf.contextLostLocked(Candidate,term){
		LOG(rf.me,rf.currentTerm,DVote,"Lost Candidate to %s, abort RequstVote",rf.role)
		return 
	}
